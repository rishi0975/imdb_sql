...
-- For number of movies released each year.
...
-- Looking out for trend month wise.(Number of movies released month wise)
...
SELECT 
    country, COUNT(country) AS Movies_Count, year
FROM
    movie
WHERE
...
(
SELECT 
    movie_id, COUNT(genre) AS No_of_Genre_Movies
FROM
    genre
GROUP BY movie_id
HAVING No_of_Genre_Movies = 1
)
SELECT COUNT(movie_id) AS No_of_Movies_One_Genre
FROM genre_cte;


-- MOVIES WITH ONE GENRE ARE 3289
...
SELECT 
    ...
    MIN(total_votes) AS Min_Tot_Votes,
    MAX(total_votes) AS Max_Tot_Votes,
...
SELECT 
    m.title, 
    MAX(r.avg_rating) AS Max_Avg_Rating, g.genre
FROM
    movie AS m
        INNER JOIN
    ratings AS r ON m.id = r.movie_id
...
FROM
    ratings AS ratings
        INNER JOIN
    movie AS movie ON ratings.movie_id = movie.id
WHERE
...
SELECT *,
CASE
	WHEN German_Votes > Italian_Votes THEN 'Yes' ELSE 'No'
...
-- Disabling 'ONLY_FULL_GROUP_BY' which is by default ON
